...
bool Posix_QextSerialPort::setConf(QList<int> index)
{
/*
QList<int> index;
index.append(ayarlar.Value("DataBits").toInt());
index.append(ayarlar.setValue("Parity").toInt());
index.append(ayarlar.setValue("FlowControl").toInt());
index.append(ayarlar.setValue("StopBits").toInt());
index.append(ayarlar.setValue("BaudRate").toInt());
*/

//DataBits
QList<DataBitsType> DataB;
DataB.append(DATA_5);
DataB.append(DATA_6);
DataB.append(DATA_7);
DataB.append(DATA_8);
setDataBits(DataB.at(index[0]));
//Parity

QList<ParityType> Parity;
Parity<<PAR_NONE<<
        PAR_ODD<<
        PAR_EVEN<<
        PAR_MARK<<               //WINDOWS ONLY
        PAR_SPACE;
setParity(Parity.at(index[1]));
...
Flow<< FLOW_OFF<<
    FLOW_HARDWARE<<
    FLOW_XONXOFF;
setFlowControl(Flow.at(index[2]));

//StopBits

QList<StopBitsType> StopBit;
StopBit<<STOP_1<<
    STOP_1_5<<               //WINDOWS ONLY
    STOP_2;
setStopBits(StopBit.at(index[3]));
//BaudRate
QList<BaudRateType> Baud;
Baud<< BAUD50<< 
    BAUD75<<                //POSIX ONLY
    BAUD110<<
    BAUD134<<              //POSIX ONLY
...
setBaudRate(Baud.at(index[4]));

return true;
// return DataB.at(index);
}
